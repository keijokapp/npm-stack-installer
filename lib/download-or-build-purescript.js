...
export default function downloadOrBuildPurescript(...args) {
	return new Observable(observer => {
		...
		const tmpSubscription = { unsubscribe() { abort.abort(); } };
		buildPurescript({ ...buildOptions, signal: abort.signal }).catch(e => { if (e.name !== 'AbortError') { observer.error(e); } });
		...
		const startBuild = feint(() => {
			...
			const abort = new AbortController();
			subscriptions.add({ unsubscribe() { abort.abort(); } });
			buildPurescript({
				...buildOptions,
				signal: abort.signal,
				progress(entry) {
					if (entry.id === 'build:complete') {
				...
				.catch(e => { sendError(e, e.id.replace('download', 'download-source')); });
		});
		...
		which('stack', async (_, stackPath) => {
			...
			try {
				stackCheckResult.version = await spawnStack({ args: ['--numeric-version'], spawnOptions: { timeout: 8000, ...options } });
			} catch (err) {
